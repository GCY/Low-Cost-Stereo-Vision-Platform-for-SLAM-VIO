...
static esp_err_t servo_handler(httpd_req_t *req){
    ...
    if (buf_len > 1) {
        ...
        if (httpd_req_get_url_query_str(req, buf, buf_len) == ESP_OK) {
            if (httpd_query_key_value(buf, "M1", M1_Value, sizeof(M1_Value)) == ESP_OK &&
                httpd_query_key_value(buf, "M2", M2_Value, sizeof(M2_Value)) == ESP_OK) {
            } else {
                free(buf);
    ...
    M1_POS = atoi(M1_Value);
    M2_POS = atoi(M2_Value);
...
static esp_err_t IMU_handler(httpd_req_t *req){
    ...
    *p++ = '{';
    ...
    p+=sprintf(p, "\"ax\":%d,\"ay\":%d,\"az\":%d,\"gx\":%d,\"gy\":%d,\"gz\":%d,\"pitch\":%.2f,\"roll\":%.2f,\"yaw\":%.2f",ax,ay,az,gx,gy,gz,pitch,roll,yaw);
    *p++ = '}';
    *p++ = 0;
    httpd_resp_set_type(req, "application/json");
    httpd_resp_set_hdr(req, "Access-Control-Allow-Origin", "*");
