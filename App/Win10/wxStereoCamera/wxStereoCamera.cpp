...
class Frame:public wxFrame
{
   public:
      ...
      void OnDisparityColormapJET(wxCommandEvent&);
   ...
   private:
      ...
      static const uint32_t WINDOW_HEIGHT = 700;
...
DECLARE_APP(App)
   ...
   EVT_MENU(ID_DISPARITY_COLORMAP_JET, Frame::OnDisparityColormapJET)
...
Frame::Frame(const wxString &title):wxFrame(nullptr,wxID_ANY,title,wxDefaultPosition,wxSize(WINDOW_WIDTH,WINDOW_HEIGHT)/*wxSize(800,600)*/,wxMINIMIZE_BOX | wxCLOSE_BOX | wxCAPTION | wxSYSTEM_MENU)
{
   CreateUI();
   ...
   esp32_cam[ID_LEFT_CAMERA] = nullptr;
   esp32_cam[ID_RIGHT_CAMERA] = nullptr;
   thread = nullptr;
...
Frame::~Frame()
{
   ...
   if(esp32_cam[ID_LEFT_CAMERA] != nullptr){
      esp32_cam[ID_LEFT_CAMERA]->StopVideoStream();
      //delete esp32_cam[ID_LEFT_CAMERA];
      //esp32_cam[ID_LEFT_CAMERA] = nullptr;
   }   

   if(esp32_cam[ID_RIGHT_CAMERA] != nullptr){
...
void Frame::CreateUI()
{
   ...
   wxMenu *disparity = new wxMenu;
   disparity->Append(ID_DISPARITY_COLORMAP_JET,wxT("ColormapJET"),wxT("ColormapJET"));
   ...
   bar->Append(disparity,wxT("Disparity"));
   SetMenuBar(bar);
...
void Frame::OnDisparityColormapJET(wxCommandEvent &event)
{
   colormap_flag ^= true;
}
