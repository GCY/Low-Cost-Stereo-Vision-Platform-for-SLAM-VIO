...
ESP32_CAM::ESP32_CAM(const std::string base_url):base_url(base_url)
{
   ...
   curl_ToF = curl_easy_init();

   curl_easy_setopt(curl_ToF,CURLOPT_VERBOSE,0);
   curl_easy_setopt(curl_ToF,CURLOPT_NOPROGRESS,1);
   curl_easy_setopt(curl_ToF,CURLOPT_TIMEOUT,COMMUNICATION_TIMEOUT);

   curl_IMU = curl_easy_init();

   curl_easy_setopt(curl_IMU,CURLOPT_VERBOSE,0);
   curl_easy_setopt(curl_IMU,CURLOPT_NOPROGRESS,1);
   curl_easy_setopt(curl_IMU,CURLOPT_TIMEOUT,COMMUNICATION_TIMEOUT);   
...
{  
   ...
   curl_easy_cleanup(curl_ToF);
   curl_easy_cleanup(curl_IMU);
...
void ESP32_CAM::SetServoPosition(int m1_pos,int m2_pos)
{
   std::stringstream ss1,ss2;
   ss1 << m1_pos;
   std::string m1_str = ss1.str();   
   ss2 << m2_pos;
   std::string m2_str = ss2.str();      

   std::string command(std::string("/servo?M1=") + m1_str + std::string("&M2=") + m2_str);
   SendStream(curl_command,command);
}

std::string ESP32_CAM::GetToFDistance()
{
   std::string buffer;

   curl_easy_setopt(curl_ToF,CURLOPT_WRITEFUNCTION,WriteCallback);
   curl_easy_setopt(curl_ToF,CURLOPT_WRITEDATA,&buffer); 

   if(SendStream(curl_ToF,std::string("/ToF"))){
   }  

   return buffer;
}

std::string ESP32_CAM::GetIMUStatus()
{
   std::string buffer;

   curl_easy_setopt(curl_IMU,CURLOPT_WRITEFUNCTION,WriteCallback);
   curl_easy_setopt(curl_IMU,CURLOPT_WRITEDATA,&buffer); 

   if(SendStream(curl_IMU,std::string("/IMU"))){
   }  
